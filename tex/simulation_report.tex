...
% TODO (open questions):
% Q1. For K=2 (pools split after the first division), is the first-event MLE
%     identical to the full-data MLE?  The K=2 estimate shows slightly higher
%     SD — explain why.  (Answer added in Section~5.7: the only gap is right-
%     censoring at T_stop for late-seeded pools.)
%
% Q2. Figure~\ref{fig:fe_vs_K} previously showed declining SD past K=50.
%     Should SD instead increase as K -> N (few pools => few first-event
%     observations)?  Investigate by extending K up to 900.
%     (Answer: SD is monotonically increasing in K; smallest at K=2,
%      largest near K=N.  K=50 was simply the highest among the old range.
%      Confirmed by extended sweep; see updated Section~5.7 and figure.)
...
The number of child pools is roughly $O(N/K)$ (Section~\ref{sec:comparison}), so
larger $K$ yields fewer first-event observations and, all else equal, larger
estimation variance.  The relationship is \emph{monotonically increasing} in $K$.
Very small $K$ does introduce right-censoring (pools seeded near $T_{\mathrm{stop}}$
may not fire before the simulation ends), but this effect is small: even at
$K = 2$ most child pools fire well before $T_{\mathrm{stop}}$, so the dominant
factor remains pool count, not censoring.

With $K = 2$ the cascade of splits is maximal — virtually every division in the
system becomes the ``first event'' of some single-cell child pool — so the
first-event MLE \emph{observes essentially the same events as the full-data MLE}.
This answers \textbf{Q1}: the gap between $K = 2$ and the full-data MLE is small
and attributable solely to right-censoring of late-seeded pools (those created
near $T_{\mathrm{stop}}$ that do not fire before the simulation ends).

For large $K$ close to $N$, very few splits occur, far fewer child pools are
created, and the first-event estimator has a small sample of first-event
observations.  Variance therefore rises sharply.  Figure~\ref{fig:fe_vs_K}
extends the sweep to $K = 900$ (close to $N = 1000$) and confirms
\textbf{Q2}: the SD increases \emph{monotonically} with $K$.  It is smallest
at $K = 2$ (nearly matching full-data precision) and grows continuously as $K$
increases.  Endpoint censoring at very small $K$ does not dominate: even at
$K = 2$ the vast majority of child pools fire their first event well before
$T_{\mathrm{stop}}$, so more pools reliably means lower variance.  The steep
upturn visible for $K > 100$ reflects the collapse of the pool-count cascade.
...
\begin{figure}[H]
  ...
  \caption{Standard deviation of the first-event MLE as a function of pool
           ...
           $x$-axis extended to $K = 900$).  Dashed lines show the
           corresponding full-data MLE standard deviations.  The SD is
           monotonically increasing in $K$: smallest at $K = 2$ (where
           nearly every system-level division is observed as a first event)
           and largest near $K = N$ (too few child pools).  The steep rise
           for $K > 100$ confirms that the first-event estimator degrades
           rapidly once the pool-count cascade collapses.  The gap between
           $K = 2$ and full-data precision is due to right-censoring of
           late-seeded pools.}
  \label{fig:fe_vs_K}
\end{figure}

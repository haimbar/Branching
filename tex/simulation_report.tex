...
%     (Answer added in Section~5.7 and confirmed by the updated figure.)
...
estimation variance.  However, the relationship is \emph{non-monotone} because
very small $K$ introduces a competing source of variance: \emph{right-censoring}.

When a child pool is seeded near $T_{\mathrm{stop}}$, it may not fire its first
division before the simulation ends.  Such a pool contributes a censored waiting
time $T_{\mathrm{stop}} - t_0$ with no event-type increment, which is
informative but less so than a completed event.  With $K = 2$ the cascade of
splits is maximal — virtually every division in the system becomes the ``first
event'' of some single-cell child pool — so the first-event MLE
\emph{observes essentially the same events as the full-data MLE}.  The only
reason its SD does not drop all the way to the full-data level is the (small)
fraction of late-seeded pools that are right-censored at $T_{\mathrm{stop}}$.
This answers \textbf{Q1}: $K = 2$ is not information-equivalent to continuous
observation solely because of endpoint censoring; if censoring were negligible,
the two estimators would coincide.
...
\textbf{Q2}: the SD is smallest at intermediate $K$ (around $K = 20$--$50$)
and increases for both very small $K$ (censoring) and very large $K$ (too few
child pools).  The curve is roughly U-shaped on the log-$K$ axis, with a
minimum well below the full-data SD only for moderate pool sizes.
...
\begin{figure}[H]
  ...
  \caption{Standard deviation of the first-event MLE as a function of pool
           ...
           corresponding full-data MLE standard deviations.  The curve is
           non-monotone: SD is minimised at intermediate $K$ ($\approx
           20$--$50$) and rises for very small $K$ (endpoint censoring)
           and for very large $K$ (too few child pools).  At $K = 2$
           the first-event MLE nearly matches full-data precision; the
           remaining gap is due to right-censoring of late-seeded pools.}
  \label{fig:fe_vs_K}
\end{figure}

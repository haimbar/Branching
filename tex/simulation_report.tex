...
\paragraph{Effect of pool size $K$.}
A smaller $K$ creates more child pools (Section~\ref{sec:comparison}),
providing more first-event observations and hence smaller estimation
variance.  Figure~\ref{fig:fe_vs_K} confirms that the SD of the
first-event estimator decreases monotonically as $K$ decreases, with
$K = 2$ approaching but not quite reaching full-data precision
(some information about the relative duration between events is discarded).
...
\begin{figure}[H]
  ...
  \caption{Standard deviation of the first-event MLE as a function of pool
           size $K$ ($N = 1000$, $p = 0.5$, 100 replicates per $K$).
           Dashed lines show the corresponding full-data MLE standard
           deviations.  Smaller $K$ yields more pools and lower estimation
           variance.}
  \label{fig:fe_vs_K}
\end{figure}

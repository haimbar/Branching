...
Y-seeded pools).  The information per pool is therefore low.

The \emph{pure-pool} scheme replaces single-cell founders with entire pure
populations.  When any pool first satisfies $\min(n_X,\,n_Y) \ge K$, it is
dissolved and replaced by two \emph{pure} pools:
\begin{itemize}
...
$P(\text{type }a) = a/(a+b)$, regardless of the current pool size.  The
pure phase ends at the first type-$b$ event, so the number of events
observed follows a $\mathrm{Geometric}(b/(a+b))$ distribution with mean
$(a+b)/b$.  For our parameters,
\[
...
\begin{itemize}
  \item \textbf{Per pool, pure-pool is far more efficient.}
        At $K = 10$ the pure-pool scheme achieves similar or lower SD per
        \emph{parameter} despite using only 30 pure pools vs.\ 355 single-cell
        pools — roughly $8\times$ more information per pool, consistent with
        the theoretical factor $(a+b)/b = 6$.

  \item \textbf{At fixed $N$, fewer pure pools are generated.}
        The min$(X,Y) \ge K$ trigger requires the mixed pool to grow until
        both types are abundant.  For $K = 10$ this yields $\approx 30$
        pure pools vs.\ $\approx 355$ single-cell pools from the standard
        scheme, so the total SD is comparable or slightly higher.

  \item \textbf{Pure-pool is preferred when the split count is the
        experimental cost.}
        If passages (pool splits) are expensive but cells are cheap, the
        pure-pool scheme delivers far more statistical information per split.
        Conversely, if total cell count is fixed, the single-cell scheme
        generates more independent pools and gives lower overall SD.

  \item \textbf{Large $K$ is inadvisable for pure-pool splitting.}
        At $K = 50$ only $\approx 7$ pure pools are generated, producing
        high variance and noticeable finite-sample bias, particularly for
        the smaller rates $b$ and $c$.
\end{itemize}
